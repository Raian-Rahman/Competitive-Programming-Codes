...
int main()
{
    ...
    while(tt--)
    {
        ...
        cin >> n;
        int arr[] = {6,10,14,16};
        if(n-arr[0]-arr[1]-arr[2]-1<0)   cout << "NO\n";
        else 
        {
            cout << "YES\n";
            if(n-arr[0]-arr[1]-arr[2]==arr[0])  cout << arr[0] << ' ' << arr[1] << ' ' << arr[3] << ' ' << n-arr[0]-arr[1]-arr[3] << endl;
            else                                cout << arr[0] << ' ' << arr[1] << ' ' << arr[2] << ' ' << n-arr[0]-arr[1]-arr[2] << endl;
        }
    }
    return 0;
}