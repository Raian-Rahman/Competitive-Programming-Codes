...
double rootDistance(double x1,double y1, double x2, double y2)
{
...
{
    double A = rootDistance(a.x,a.y,b.x,b.y);
    double B = rootDistance(b.x,b.y,c.x,c.y);
    double C = rootDistance(a.x,a.y,c.x,c.y);
    double res = acos(( C*C-A*A - B*B )/(-2.0*A*B));
   // if(res!=res)    return 1.0*INT_MAX;
    //else
        return res;
}
int main()
{
    ...
    while(t--)
    {
     ...
     //   }
        ...
        {
            double minAngle = (getAngle(points[points.size()-1],points[0],points[1])*180)/PI ;
            for(int i=1;i<points.size();i++)
            {
                double res =  (getAngle(points[(i-1)%n],points[(i)%n],points[(i+1)%n])*180)/PI ;
                minAngle=min(minAngle,res);
               // cout << minAngle << ' '
            }
            CASE(T-t);
            FIXED(6);
